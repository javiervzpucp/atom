...
def extract_text_from_docx(docx_path):
    ...
    return text[:1000]  # Reducimos el tamaño para mejorar rendimiento
...
def get_embeddings_batch(texts):
    """Obtiene los embeddings de OpenAI en batch para mejorar rendimiento."""
...
def find_best_match(column_name, column_values, atom_embeddings):
    ...
    combined_text = cleaned_column_name + " " + " ".join(map(str, column_values[:2]))  # Reducimos tamaño
    ...
    # Ordenar y seleccionar las 3 mejores coincidencias
    top_matches = sorted(similarity_scores.items(), key=lambda x: x[1], reverse=True)[:3]
    st.write(f"Top 3 similitudes para {column_name}:", top_matches)
